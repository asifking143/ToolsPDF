/**
 * Logic for JPG to PDF Tool (Similar to image-to-pdf but optimized file acceptance constraint)
 */
...
document.addEventListener('DOMContentLoaded', () => {
    ...
    function handleDrop(e) {
        ...
        const files = Array.from(dt.files).filter(f => f.type.includes('jpeg') || f.type.includes('jpg'));
        if(files.length > 0) handleFiles(files);
        else alert('Please upload only JPG/JPEG files.');
    }
    ...
    function handleFiles(files) {
        currentFiles = files;
        fileNameDisplay.textContent = `${files.length} JPG file(s) selected`;
        ...
        try {
            ...
            for (let i = 0; i < currentFiles.length; i++) {
                ...
                try {
                    ...
                    console.error("Not a JPG", e);
                    continue; // Skip non-JPGs gracefully if any slipped through
                }
