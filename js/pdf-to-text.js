...
document.addEventListener('DOMContentLoaded', () => {
    ...
    let downloadName = 'extracted_text.txt';
    ...
    async function handleFile(file) {
        ...
        downloadName = file.name.replace('.pdf', '.txt');
    ...
    processBtn.addEventListener('click', async () => {
        ...
        try {
            ...
            for (let i = 1; i <= totalPages; i++) {
                ...
                // Construct string from textContent items
                let pageText = textContent.items.map(item => item.str).join(' ');
                
                // Try to infer line breaks based on Y positions (simplistic approach)
                let lastY = -1;
            ...
            processedBlob = new Blob([fullText], { type: 'text/plain;charset=utf-8' });
            
            textPreview.textContent = fullText.substring(0, 1000) + (fullText.length > 1000 ? '\n\n... (preview truncated)' : '');
